...
impl BitReader {
  pub fn new(buffer: BytePtr) -> Self {
    ...
    let buffered_values = read_num_bytes!(u64, num_bytes, buffer.slice_all());
    BitReader {
  ...
  #[inline]
  pub fn reset(&mut self, buffer: BytePtr) {
    self.buffer = buffer;
    self.total_bytes = self.buffer.len();
    let num_bytes = cmp::min(8, self.total_bytes);
    self.buffered_values = read_num_bytes!(u64, num_bytes, self.buffer.slice_all());
    self.byte_offset = 0;
    self.bit_offset = 0;
  }
  ...
  pub fn get_value<T: Default>(&mut self, num_bits: usize) -> Result<T> {
    ...
    if self.bit_offset >= 64 {
      ...
      self.buffered_values = read_num_bytes!(
        u64, bytes_to_read, self.buffer.slice_start_from(self.byte_offset));
  ...
  pub fn get_aligned<T: Default>(&mut self, num_bytes: usize) -> Result<T> {
    ...
    let v = read_num_bytes!(T, num_bytes, self.buffer.slice_start_from(self.byte_offset));
    self.byte_offset += num_bytes;
    ...
    self.buffered_values = read_num_bytes!(
      u64, bytes_remaining, self.buffer.slice_start_from(self.byte_offset));
    Ok(v)
  }
...
mod tests {
  ...
  use super::*;
  use super::super::memory::BytePtr;
  ...
  fn test_bit_reader_get_value() {
    ...
    let mut bit_reader = BitReader::new(BytePtr::new(buffer));
  ...
  fn test_bit_reader_get_aligned() {
    // 01110101 11001011
    let buffer = BytePtr::new(vec!(0x75, 0xCB));
  ...
  fn test_put_value_rand_numbers(total: usize, num_bits: usize) {
    ...
    for i in 0..total {
      let result = reader.get_value::<u64>(num_bits);
      assert!(result.is_ok(), "[{}]: get_value() returned error:\n {}", i, result.unwrap_err());
      let v = result.unwrap();
  ...
  fn test_put_aligned_rand_numbers<T>(total: usize, num_bits: usize)
    ...
    for i in 0..total {
      ...
      if i % 2 == 0 {
        ...
        let result = reader.get_aligned::<T>(aligned_value_byte_width);
        assert!(result.is_ok(), "[{}]: get_aligned() returned error:\n {}", i, result.unwrap_err());
        let v = result.unwrap();
  ...
  fn test_put_vlq_int() {
    ...
    let mut reader = BitReader::new(writer.consume());
    for i in 0..total {
      let result = reader.get_vlq_int();
      assert!(result.is_ok(), "[{}]: get_vlq_int() returned error:\n {}", i, result.unwrap_err());
      let v = result.unwrap();
  ...
  fn test_put_zigzag_vlq_int() {
    ...
    let mut reader = BitReader::new(writer.consume());
    for i in 0..total {
      let result = reader.get_zigzag_vlq_int();
      assert!(result.is_ok(), "[{}]: get_zigzag_vlq_int() returned error:\n {}", i, result.unwrap_err());
      let v = result.unwrap();
